...
.constant('platformSettings', {
    realHasWebRTC: navigator.msLaunchUri ? false : navigator.getUserMedia || navigator.mozGetUserMedia || navigator.webkitGetUserMedia || false, // changes here should be duplicated below
    hasWebRTC: navigator.msLaunchUri ? false : navigator.getUserMedia || navigator.mozGetUserMedia || navigator.webkitGetUserMedia || false,
...
.constant('applicationSettings', angular.extend({
    ...
    // Uncomment to enable Firefox screensharing. Note this requires the webapp FQDN to be in the about:settings whitelist
    // firefoxScreensharing: true,
