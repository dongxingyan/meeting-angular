...
.factory('mediaDevicesService', function($log, $q) {
                ...
                switch (device.kind) {
                    ...
                    case 'videoinput':
                        result.video.push({
                            id: device.id || device.deviceId,
                            kind: 'video',
                            label: device.label || 'Camera ' + (result.length + 1)
                        });
                        break;
                    case 'audiooutput':
                        result.output.push({
                            id: device.id || device.deviceId,
                            kind: 'output',
                            label: device.label || 'Output ' + (result.length + 1)
                        });
                        break;
                }
            }
    ...
    service.enumerateDevices = function() {
        ...
        function enumerateDevicesSuccess(result) {
            ...
            devices.resolve(parseDevices(result));
        }
    ...
    service.getInvalidDevices = function(devices) {
        ...
        return service.enumerateDevices()
        .then(function(validDevices) {
            ...
            if (results.length) {
                $log.error('Invalid device IDs:', results);
            }
