...
angular.module('pexapp')
    ...
    .controller('MainController', function ($rootScope,
        ...
        // drpeng扩展工具包
        var drpengExt = $rootScope['drpengExtend'];

        drpengExt.mainScope = $scope;
        ...
        if (navigator.userAgent.indexOf("Firefox") != -1) {
            ...
            if (applicationSettings.firefoxScreensharing === true) {
        ...
        $scope.loginApp = function (alias, displayName, skipDeviceSelectionDialog) {
            ...
            if ($localStorage.cameraSourceId) {
                devices.push({kind: 'video', id: $localStorage.cameraSourceId});
            }
            if ($localStorage.microphoneSourceId) {
                devices.push({kind: 'audio', id: $localStorage.microphoneSourceId});
            }
            if ($localStorage.audioOutputId) {
                devices.push({kind: 'output', id: $localStorage.audioOutputId});
            }
            mediaDevicesService.getInvalidDevices(devices)
                .then(function (invalidDevices) {
                    angular.forEach(invalidDevices, function (invalidDevice) {
                        switch (invalidDevice.kind) {
                            case 'video':
                                $localStorage.cameraSourceId = null;
                                break;
                            case 'audio':
            ...
            $scope.connection = {
                ...
                participantAdd: function (uri, protocol, role, userParams) {
                    ...
                    if (typeof userParams === 'string') {
                        userParams = {presentation_uri: userParams};
                    }
                    $scope.sendKeenEvent(
                        'dialParticipant', {
                            protocol: protocol,
        ...
        $scope.login = function (alias, displayName, pin, token, extension) {
            ...
            if ($scope.call) {
                ...
                var uri = '';
                if ($localStorage.defaultDomain != 'tp.cloudp.cc') {
                    uri = $localStorage.defaultDomain && alias.search('@') < 0 ? alias + '@' + $localStorage.defaultDomain : alias;
                } else {
                    uri = alias
                }
        ...
        $scope.disconnect = function (navigate) {
            ...
            $scope.$emit('drpeng::disconnect', {navigate: navigate});
        ...
        $scope.$on('call::participantAdd', function (event, participant) {
            ...
            console.warn('新增参会者：', participant);
        ...
        $scope.$on('call::screenShareMissing', function (event, stage) {
            // drpeng：拦截屏幕分享失效事件。
            var extFn;
            console.debug(drpengExt);
            if (drpengExt && drpengExt['screenShareExtend']) {
                extFn = drpengExt.screenShareExtend;
                if (extFn()) {
