class WebSocketService {
    static
    $inject = ["$rootScope", "$log", "$sce", "$q", "$localStorage", "$window", "$timeout", "$location", "$translate", "platformSettings", "applicationSettings", "defaultUserSettings", "serverSettings", "toast", "modal", "callHistory", "Call", "srvService", "toggleService", "dialHistory", "flashVideo", "mediaDevicesService", "reportingService"]
    ...
    connect(alias, uuid) {
        ...
        this.uuid = uuid;
        let websocket,
            ws_url = "wss://" + this.applicationSettings.webSocketUrl + "/cloudpServer/websocket/connect?info=" + alias + "_" + uuid;
        // 检查是否已经存在Websocket链接
        if (this.ws) {
    ...
    handUpListReceiveListener: (list: string[]) => void;
    addHandUpListReceiveListener(callback: (list: string[]) => void) {
